...
namespace ft
{
    ...
    public:
        ...
        vector(size_type n, const value_type &value = value_type(), const allocator_type &alloc = allocator_type(), typename ft::enable_if<ft::is_integral<size_type>::value, size_type>::type * = 0)
        ...
        void reserve(size_type n)
        {
            ...
            {
                this->_begin[this->_size] = ori_begin[this->_size];
            }
                    ...
                    typename ft::enable_if<!ft::is_integral<InputIterator>::value>::type * = 0)
        {
            ...
            {
                this->_alloc.construct(&this->_begin[this->size()], *first);
                this->_size++;
            }
            this->_size = diff;
        }
        void assign(size_type n, const value_type &u, typename ft::enable_if<ft::is_integral<size_type>::value, size_type>::type * = 0)
        {
            this->clear();
            if (this->_capa < n)
                this->reserve(n);
        ...
        void push_back(const value_type &x)
        {
            ...
            insert(this->begin() + this->size(), 1, x);
        }
        ...
        void insert(iterator position, size_type n, const value_type &x, typename ft::enable_if<ft::is_integral<size_type>::value, size_type>::type * = 0)
        {
            // find iter index
            size_type start = position.base() - this->_begin;

            const size_type capa = capacity();
            size_type  new_size = capa + ((capa > n) ? (capa) : n);
        ...
        iterator erase(const_iterator first, const_iterator last)
        {
            ...
            size_type start = first.base() - this->_begin;
            size_type len = last.base() - first.base();
            size_type i = 0;

            for (; this->_begin + len + start + i < this->end().base(); i++)
            {
                *(this->_begin + start + i) = *(this->_begin + start + i + len);
               
            }

            for (; len > 0; len--)
            {
                this->_alloc.destroy(&this->_begin[len]);
                this->_size--;
            }
